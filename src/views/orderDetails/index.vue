<template>
  <div class="dashboard-container">
    ...
    <div class="container" :class="{ hContainer: tableData.length }">
      ...
      >
        <el-table-column type="selection" width="25" v-model="selectAll"/>
          ...
          <template slot-scope="{ row }">
            ...
            <div class="middle">
              ...
              <el-button
                ...
                :disabled="buttonDisabled(row.status)"
              >
                取消
              </el-button>
              <!-- <el-button
...
export default class extends Vue {
  ...
  private selectAll=false // 全选状态，默认为未选中
  private selectedRows = [] // 已选中的行
  ...
  getOrderListBy3Status() {
    getOrderListBy({})
      ...
      .catch((err) => {
        console.error('getOrderList err:', err)
  ...
  init(activeIndex: number = 0, isSearch?) {
    ...
    getOrderDetailPage({ ...params })
      ...
      .catch((err) => {
        console.error('getOrderDetailPage err:', err)
        // this.$message.error('请求出错了：' + err.message)
      })
  }
    ...
    orderAccept({ id: this.orderId })
      .then((res) => {
        if (res.data.code === 1|| res.data.code === true) {
          ...
          console.error('res.data.msg:', res.data.msg)
          // this.$message.error(res.data.msg)
        }
      })
      .catch((err) => {
        console.error('orderAccept请求出错了:', err);
        // this.$message.error('请求出错了：' + err.message)
      })
  }
  ...
  batchAccept() {
    ...
    // 遍历 selectedRows 数组，执行接单操作
    this.selectedRows.forEach((row) => {
      // 执行接单操作
      this.orderAccept(row);
    });
  }
  ...
  cancelOrDeliveryOrComplete(status: number, id: string) {
    ...
    ;(status === 3 ? deliveryOrder : completeOrder)(params)
      ...
      .catch((err) => {
        console.error('cancelOrDeliveryOrComplete请求出错了:', err);
        // this.$message.error('请求出错了：' + err.message)
      })
  }
  ...
  batchDelivery() {
    if (this.selectedRows.length === 0) {
      this.$message.warning('请先选择要派送的订单');
      return;
    }
    ...
    orderIds.forEach(orderId => {
      this.cancelOrDeliveryOrComplete(3, orderId);
    });
  }

  batchFinish() {
    if (this.selectedRows.length === 0) {
      this.$message.warning('请先选择已经完成的订单');
      return;
    }
    ...
    // 调用派送函数进行批量派送
    orderIds.forEach(orderId => {
      this.cancelOrDeliveryOrComplete(4, orderId);
    });
  }
  ...
  private handleSelectionChange(selectedRows) {
